$(document).ready(function () {
  ...
  function setRandomSize() {
    ...
    imgHeight = img.outerHeight(100);
    imgWidth = img.outerWidth();

    if (window.innerHeight > window.innerWidth) {
      //Generate random Width % [10, 60]
      var randomWidth = randomIntFromInterval(50, 50);
      $(this).css({
        width: randomWidth + "%",
      });
    } else {
      //Change Height to [100, 330] randomly
      var randomHeight = randomIntFromInterval(100, 100);
  ...
  function setRandomPosition() {
    ...
    var randomTop = randomIntFromInterval(50, 50);
    //Generate randon Left % [0, 85]
    var randomLeft = randomIntFromInterval(50, 50);
    $(this).css({
      top: randomTop + "%",
      left: randomLeft + "%",
    });
